...
local function init_allowed_items()
    for _, item in pairs(game.item_prototypes) do
        if not config.allowed_items[item.name] then
            config.allowed_items[item.name] =
            {
                starting_value = 0,
                required_area = 0,
                surface_area = 0,
...
local function handle_circuit_interfaces()
    ...
    for index, interface in pairs(vlayer_data.entity_interfaces.circuit) do
        if not interface.valid then
            ...
            for item_name, count in pairs(vlayer_data.storage.items) do
                if not config.allowed_items[item_name].hidden then
    ...
    Event.on_init(init_allowed_items)
