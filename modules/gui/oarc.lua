...
local oarc_player_settings = {}
Global.register(oarc_player_settings, function(tbl)
    oarc_player_settings = tbl
end)

function oarc.get_player_settings(player)
    if oarc_player_settings[player.name] then
        return oarc_player_settings[player.name]
    else
        return false
    end
end
...
Gui.element{
    ...
    tooltip = {'oarc.toggle-section-tooltip'},
...
:on_click(function(_, element, _)
    ...
    if Gui.toggle_visible_state(flow) then
        ...
        element.tooltip = {'oarc.toggle-section-collapse-tooltip'}
    else
        element.sprite = 'utility/expand_dark'
        element.hovered_sprite = 'utility/expand'
        element.tooltip = {'oarc.toggle-section-tooltip'}
    end
end)
...
Gui.element(function(definition, parent, section_name, table_size)
    ...
    parent,
    {'oarc.toggle-section-caption', section_name},
    {'oarc.toggle-section-tooltip'},
    true,
    section_name..'-header'
)
...
Gui.element(function(_, parent, option_name)
    return parent.add{
        ...
        tooltip = {'oarc.toggle-option-on-tooltip', option_name},
...
:on_click(function(player, element, _)
    ...
    if not oarc_player_settings[player.name] then return end
    local setting = oarc_player_settings[player.name][option_name]
    if not setting then return end
    if setting.enabled then
        setting.enabled = false
        element.sprite = 'utility/close_black'
        element.style = 'shortcut_bar_button_red'
        element.tooltip = {'oarc.toggle-option-on-tooltip', option_name}
        game.player.print({'oarc.toggle-option-off', option_name})
    else
        setting.enabled = true
        element.sprite = 'utility/confirm_slot'
        element.style = 'shortcut_bar_button_green'
        element.tooltip = {'oarc.toggle-option-off-tooltip', option_name}
        game.player.print({'oarc.toggle-option-on', option_name})
    end
...
Gui.element(function(definition, parent, option_name, caption, tooltip, table_size)
    ...
    local setting = label(
    parent,
    {'oarc.toggle-option-caption', caption},
    true,
    option_name..'-option',
    nil,
    tooltip
...
local function format_item_name(item)
    return {'oarc.item-name', rich_img('item', item), game.item_prototypes[item].localised_name}
end
...
Gui.left_toolbar_button('utility/show_recipe_icons_in_map_view', {'oarc.main-tooltip'}, oarc_container, function(player)
    return Roles.player_allowed(player, 'gui/oarc')
end)

local function redraw_item_list()
    local item_list = vlayer.get_items()
    for _, player in pairs(game.connected_players) do
        local frame = Gui.get_left_element(player, oarc_container)
...
Event.add(defines.events.on_player_created, function(event)
    ...
    spawn_choices(player.gui.center)
    oarc_player_settings[player.name] = config.player_settings
end)
