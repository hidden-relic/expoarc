...
Selection.on_selection(SelectionArtyArea, function(event)
    ...
    for _, e in pairs(player.surface.find_entities_filtered({area=area, type={'unit-spawner', 'turret'}, force='enemy'})) do
        ...
        if not skip then
            player.surface.create_entity{name='artillery-flare', position=e.position, force=player.force, life_time=300, movement={0, 0}, height=0, vertical_speed=0, frame_speed=0}
